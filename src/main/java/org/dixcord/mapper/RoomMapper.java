...
public interface RoomMapper {
	 ...
	 // 모든 방 리스트 가져오기
	   ...
	   // 방 번호로 방 맴버 리스트 가져오기
	   public List<RoomMemberVO> getChatMember(int roomNumber);
	   ...
	   // 유저가 참여한 방의 권한 가져오기
	   public String getRoomAuth(RoomMemberVO rmvo);
	
	   //방장이 나가면 방장 다음으로 들어온 사람에게 방장 권한 이전
	   public int updateHost(int roomNumber);
	   
	   //방 나간 후 참여 목록에서 유저 삭제(방 나가기)
	   public int exitRoom(int userCode);
	   
	   //dico_room 테이블 업데이트(userCode:개설자 코드)
	   public int updateCreator(RoomMemberVO second);
	   ...
	   //유저 닉네임들 불러오기
	   public List<UserVO> getUserNickName(int userCode);
