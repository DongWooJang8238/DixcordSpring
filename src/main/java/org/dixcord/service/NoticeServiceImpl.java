...
public class NoticeServiceImpl implements NoticeService {
	...
	public int insert(NoticeVO vo) {
	    // 게시글 등록
	    log.info("게시글 번호 확인 전 : " + vo.getIdx());
	    int result = mapper.insert(vo);
	    log.info("게시글 등록 후 idx 번호 : " + vo.getIdx());
	    if (vo.getAttachList().size() > 0) { // 파일 정보가 있을 때만 첨부파일 등록
	        vo.getAttachList().forEach(navo -> {
	        	navo.setIdx(navo.getIdx());
	        	int count = attachMapper.insertImg(navo);
	        });
	    }
